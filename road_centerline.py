...
class RoadCenterlineClient:
    ...
    def fetch_paths(self, bounds: Bounds) -> list[list[tuple[float, float]]]:
        ...
        while url:
            if url in seen_urls:
                raise RoadCenterlineError("Wegdeel-hartlijnen server herhaalt dezelfde vervolgpagina.")
            seen_urls.add(url)
            page_count += 1
            if page_count > self.MAX_PAGES:
                raise RoadCenterlineError(
                    f"Wegdeel-hartlijnen overschrijden de veilige paginalimiet ({self.MAX_PAGES})."
                )
            try:
