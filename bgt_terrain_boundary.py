...
class BgtTerrainBoundaryClient:
    ...
    def fetch_paths(self, bounds: Bounds) -> list[list[tuple[float, float]]]:
        ...
        while url:
            if url in seen_urls:
                raise BgtTerrainBoundaryError("BGT terreinserver herhaalt dezelfde vervolgpagina.")
            seen_urls.add(url)
            page_count += 1
            if page_count > self.MAX_PAGES:
                raise BgtTerrainBoundaryError(
                    f"BGT terreinranden overschrijden de veilige paginalimiet ({self.MAX_PAGES})."
                )
            try:
